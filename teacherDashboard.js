class TeacherDashboard {
    ...
    initializeCharts() {
        ...
        this.charts.subjectPerformance = new Chart(
            document.getElementById('subject-performance-chart'),
            {
                type: 'radar',
                data: {
                    labels: ['Science', 'Math', 'Language'],
                    datasets: [{
                        label: 'Class Average',
                        data: [0, 0, 0],
        ...
        this.charts.weeklyProgress = new Chart(
            document.getElementById('weekly-progress-chart'),
            {
                type: 'line',
                data: {
    ...
    updateCharts() {
        ...
        this.charts.subjectPerformance.data.datasets[0].data = [
            subjectPerformance.science,
            subjectPerformance.math,
            subjectPerformance.language
        ];
        this.charts.subjectPerformance.update();

        // Update weekly progress chart
        const weeklyProgress = this.calculateWeeklyProgress();
        this.charts.weeklyProgress.data.datasets[0].data = weeklyProgress;
        this.charts.weeklyProgress.update();
    }

    calculateSubjectPerformance() {
        const totals = { science: 0, math: 0, language: 0 };
        const counts = { science: 0, math: 0, language: 0 };

        this.students.forEach(student => {
            if (!student.progress?.subjects) return;

            Object.entries(student.progress.subjects).forEach(([subject, data]) => {
                if (data.score) {
                    totals[subject] += data.score;
                    counts[subject]++;
                }
            });
        });
        ...
        return {
            science: counts.science ? Math.round(totals.science / counts.science) : 0,
            math: counts.math ? Math.round(totals.math / counts.math) : 0,
            language: counts.language ? Math.round(totals.language / counts.language) : 0
        };
    }
    ...
    calculateWeeklyProgress() {
        ...
        this.students.forEach(student => {
            if (!student.progress?.dailyScores) return;

            days.forEach((day, index) => {
                if (student.progress.dailyScores[day]) {
                    dailyScores[index] += student.progress.dailyScores[day];
                    dailyCounts[index]++;
                }
            });
        });
        ...
        return dailyScores.map((score, index) => 
            dailyCounts[index] ? Math.round(score / dailyCounts[index]) : 0
        );
    }
